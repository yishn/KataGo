...
pub async fn position_eval(
  ...
  nninputs::fill_row(
    board, hist, next_player, nn_x, nn_y, nc, ng,
...
mod tests {
  ...
  fn wgpu_position_eval_value_probs() {
    ...
    pollster::block_on(async {
      ...
      let sum = nn.white_win_prob + nn.white_loss_prob + nn.white_no_result_prob;
  ...
  fn wgpu_position_eval_score_finite() {
    ...
    pollster::block_on(async {
      ...
      let hist = BoardHistory::new(&board, Player::Black, Rules::default(), 0);
      let nn = position_eval(&ev, &board, &hist, Player::Black).await;
      assert!(nn.white_score_mean.is_finite(), "white_score_mean not finite");
  ...
  fn wgpu_position_eval_empty_board_balanced() {
    ...
    pollster::block_on(async {
      ...
      let hist = BoardHistory::new(&board, Player::Black, Rules::default(), 0);
      let nn = position_eval(&ev, &board, &hist, Player::Black).await;
      assert!(nn.white_win_prob  < 0.9, "white win prob suspiciously high: {}", nn.white_win_prob);
      assert!(nn.white_loss_prob < 0.9, "white loss prob suspiciously high: {}", nn.white_loss_prob);
    });
  }
